...
export function renderEngine(filePath:string, optionsOBJ:any, callback:(e: any, rendered?: string) => void):void
{
    var rendered:string;
	...
	fs.readFile(filePath, function(err, data)
	{
		...
		if(optionsOBJ.skeleton != null)
		{
			fs.readFile(optionsOBJ.skeleton, function(err, data)
			{
				...
				var skeleton:string = data.toString();
				...
				var titleText:string = titleToAdd != null ? titleToAdd[1] : "";


				var contentToAdd = rendered.match(/###\s*(?:Content|content)\s*=\s*([\s\S]*?)\s*###/);
				var contentText:string = contentToAdd != null ? contentToAdd[1] : "";
